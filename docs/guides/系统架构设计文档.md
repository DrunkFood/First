...
graph TB
    ...
    subgraph "前端层"
        SupportFrontend[支撑中心前端<br/>端口:3060<br/>Vue 3 + TypeScript<br/>Vite + Element Plus]
...
|---------|------|------|------|-------------|
| `ele-ai-tender-support-frontend` | 前端 | 3060 | 支撑中心管理界面 | - |
...
| `ele-ai-tender-core` | 后端服务 | 8082 | 项目管理、业务需求编制、评审项管理、文档生成 | `tb_*` |
...
- `TemplateConfigController`: `/api/v1/template-configs/*`
- `KnowledgeConfigController`: `/api/v1/knowledge-configs/*`
- `StatisticsController`: `/api/v1/statistics/*`
- `ModelConfigController`: `/api/v1/model-configs/*`
- `ModelRouteRuleController`: `/api/v1/model-routes/*`
- `AccessLogController`: `/api/access-logs`
- `OperationLogController`: `/api/v1/operation-logs`
...
- `SysParameterController`: `/api/v1/sys-params/*`
...
- 文件 SHA-256 查询:`GET /api/file/info/sha256/{sha256}`
- 文件删除:`DELETE /api/file/delete/{fileId}`
- Word结构解析:`GET /api/file/structure/{fileId}`
- 文档生成:`POST /api/file/generate-doc`
- 文档修复:`POST /api/file/fix-doc`
...
**关键数据表**:
- `tb_project`: 项目表
- `tb_project_version`: 项目版本表
- `tb_requirement`: 业务需求表
- `tb_project_review_item`: 评审项表
- `tb_project_template`: 项目模板快照表
- `tb_detection_record`: 检测记录表
- `tb_policy_file`: 用户政策文件表
...
- `ele-ai-tender-interaction-autoconfigure`: 自动装配 + 拦截器 + 回调Controller
- `ele-ai-tender-interaction-spring-boot-starter`: Starter 聚合 POM

**固定接口**(业务系统接入后暴露):
- `POST /api/eleAiTender/interaction/callbacks/ai-task-result` — AI任务结果回调

**业务系统只需实现的 SPI**:
- `InteractionAiTaskResultReceiveService`: AI任务结果接收
- `InteractionEventLogger`: （可选覆盖）交互事件日志

**出站能力**(统一客户端):
- `AiExternalAuthClient`: 获取外部 token
- `AiExternalUserInfoClient`: 获取当前用户
- `AiTaskClient`: 创建/查询 AI 任务
- `AiFileClient`: 文件查询/下载/上传
...
GET    /api/v1/projects/{id}/phase         # 获取项目当前阶段
PUT    /api/v1/projects/{id}/phase         # 推进项目阶段
...
POST   /api/v1/ai/match/manual             # 手动选择匹配
POST   /api/v1/ai/model-test/deepseek      # 测试 DeepSeek 模型连通性
...
```
POST   /api/v1/detection/submit/{projectId}        # 提交检测
...
POST   /api/v1/review-items/{projectId}/generate  # AI生成评审项
POST   /api/v1/review-items/batch          # 批量创建评审项
PUT    /api/v1/review-items/batch          # 批量更新评审项
PUT    /api/v1/review-items/{projectId}/replace   # 全量替换评审项
```

### 8.7 模板管理

```
# 支撑中心模板配置（/api/v1/template-configs）
...
| 表名 | 说明 | 关键字段 |
|------|------|----------|
| `tb_project` | 项目表 | 编号、名称、类别、类型、预算、状态、当前阶段、模板关联、需求内容 |
| `tb_project_version` | 项目版本表 | 版本号、内容快照JSON、变更说明 |
| `tb_requirement` | 业务需求表 | 需求名称、需求描述、匹配模式、匹配度、内容 |
| `tb_project_template` | 项目模板快照表 | 模板内容、结构定义JSON、版本号 |
| `tb_detection_record` | 检测记录表 | 检测类型、内容快照、结果JSON、状态 |
| `tb_project_review_item` | 评审项表 | 三级嵌套结构:parent_id + level |
| `tb_policy_file` | 用户政策文件表 | 文件名、适用类别、文件ID、状态 |
| `ai_task` | AI任务表 | 任务类型、状态、请求参数、结果JSON、同步状态 |
| `ai_knowledge_document` | 知识库文档表 | 文档类别、向量集合、向量ID列表 |
| `ai_response_log` | AI响应日志表 | 模型信息、Token用量、响应内容 |
| `ai_content_feedback` | AI内容反馈表 | 反馈类型、场景、目标ID |
| `sup_model_config` | AI模型配置表 | 模型类型、API端点/密钥、参数JSON、Token用量 |
| `sup_template` | 招标文件模板表 | 模板名称、项目类别、项目类型、内容、结构定义、评审配置 |
...
# 后端 - 首次启动需先安装依赖模块(在 ele-ai-tender-system/ 目录下)
...
> **注意**: 后端启动必须在**子模块目录**下执行 `mvn spring-boot:run`，不能在父POM目录用 `-pl` 启动。首次启动前须先 `mvn install` 安装公共模块到本地仓库。
...
| 前端编码规范 | [docs/rules/FRONTEND_CONVENTIONS.md](../rules/FRONTEND_CONVENTIONS.md) |
| 核心业务模块规范 | [docs/rules/CORE_MODULE_SPEC.md](../rules/CORE_MODULE_SPEC.md) |
| AI服务模块规范 | [docs/rules/AI_MODULE_SPEC.md](../rules/AI_MODULE_SPEC.md) |
...
| 检测全链路规范 | [docs/rules/DETECTION_FLOW_SPEC.md](../rules/DETECTION_FLOW_SPEC.md) |
| 阶段流程控制器规范 | [docs/rules/PHASE_FLOW_SPEC.md](../rules/PHASE_FLOW_SPEC.md) |

### 12.2 使用指南

| 指南 | 路径 |
|------|------|
| 外部系统对接指南 | [docs/guides/EXTERNAL_SPI_INTEGRATION_GUIDE.md](EXTERNAL_SPI_INTEGRATION_GUIDE.md) |
...
| 项目 | Project (tb_project) | 招标项目 |
| 项目版本 | ProjectVersion (tb_project_version) | 项目版本快照 |
| 业务需求 | Requirement (tb_requirement) | 招标需求描述 |
| 项目模板 | ProjectTemplate (tb_project_template) | 项目模板快照 |
| 招标文件模板 | Template (sup_template) | 招标文件模板配置 |
| 知识库文档 | KnowledgeDocument (ai_knowledge_document) | 向量化知识文档 |
| 检测记录 | DetectionRecord (tb_detection_record) | 智能检测结果 |
| 评审项 | ReviewItem (tb_project_review_item) | 评审配置项 |
| AI任务 | AiTask (ai_task) | AI异步任务 |
| AI响应日志 | AiResponseLog (ai_response_log) | AI调用记录 |
| AI内容反馈 | AiContentFeedback (ai_content_feedback) | AI内容反馈 |
| AI模型配置 | ModelConfig (sup_model_config) | 模型API配置 |
...
**检测类型**: SENSITIVE_WORD(敏感词) / TYPO(错别字) / POLICY_REVIEW(政策文件审查) / FORMAT_CHECK(格式规范)

**AI模型类型**: LOCAL(本地微调) / CLOUD(云端大模型) / PRIVATE(私有化部署)

**AI使用场景**: GENERATION(生成) / OPTIMIZATION(优化) / DETECTION(检测) / CHAT(对话)

**需求来源**: REFERENCE(引用已有需求) / SYSTEM_GENERATE(系统生成)

**匹配模式**: AUTO_MATCH(自动匹配) / MANUAL_SELECT(手动选择) / UPLOAD(上传)

**评审方式**: INTELLIGENT(智能评标) / MANUAL(人工评审)

**计分模式**: SCORE(分值模式) / WEIGHT(权重模式)
...
2. **模块化设计**:6个后端顶级模块 + 2个前端项目,职责清晰,依赖关系明确
