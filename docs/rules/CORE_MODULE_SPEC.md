...
| 表名 | 用途 |
|------|------|
| `tb_project` | 项目表：project_code / project_name / project_category / project_type / budget / status / current_phase / progress / template_id / requirement_id / requirement_content / generated_file_id |
| `tb_project_version` | 项目版本表：project_id / version_no / content_snapshot(JSON) / change_description |
| `tb_requirement` | 业务需求表：requirement_name / project_category / project_type / budget / requirement_description / match_mode / matched_file_id / matched_similarity / uploaded_file_id / content / auto_save_content / status / progress（**无 projectId**，需求与项目通过 `tb_project.requirement_id` 单向关联） |
