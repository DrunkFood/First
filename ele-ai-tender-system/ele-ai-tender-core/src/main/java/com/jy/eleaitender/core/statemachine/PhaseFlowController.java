...
public class PhaseFlowController {
    ...
    public void advancePhase(AiProject project, ProjectPhase target, Map<String, Object> context) {
        ...
        // 2. 校验项目下无活跃AI任务（正在执行的任务完成前禁止推进）
        if (aiTaskService.hasActiveTasks(project.getId())) {
            throw new BusinessException(ResponseCode.PROJECT_PHASE_ACTIVE_TASK);
        }

        // 3. 执行当前阶段的 onExit（离开前校验是否可完成）
