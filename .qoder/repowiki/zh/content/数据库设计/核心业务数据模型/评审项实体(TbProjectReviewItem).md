...
## 核心组件
- 实体与基类
  - TbProjectReviewItem：评审项领域模型，包含项目归属、父子关系、层级、名称内容、排序、评审类型、分值权重、主客观性与必审标记等字段，并提供便捷方法用于标准文本拼装。
...
- 异步生成与汇总
  - AiTaskResultSyncHandler：解析 AI 生成的评审项 JSON，结合配置进行占位节点插入、分数归一化、主观性清洗、父子关系回填与落库。
  - DocumentDataAssembler：将评审项树组装为文档摘要或合规清单，支持权重模式与分数模式的标签展示。
...
- 评分与权重
  ...
  - weight：权重百分比（一级节点常用）
- 主客观性与必审
  - subjectivity：OBJECTIVE/SUBJECTIVE，可按配置决定是否区分
...
- 文档组装
  - 将评审项树转换为 Markdown 或摘要列表；支持权重模式与分数模式下的不同标签展示
...
- 同步处理
  - 调度器解析 AI 返回的 JSON，按配置过滤未启用类型、插入占位节点、归一化分数、清理主观性字段
...
## 结论
TbProjectReviewItem 采用简洁而稳健的自引用树形结构，配合 level、sort_order 与 review_type 的组合，既能满足多级评审项的组织需求，又能支撑多种评分模式与主客观性区分。服务层在创建、更新、删除与批量操作中实现了严格的层级与归属校验，调度器与文档装配器进一步增强了自动化与可读性。通过合理的索引设计与批量/全量替换策略，系统在易用性与性能之间取得良好平衡。
