...
## 更新摘要
**所做更改**   
- 更新了AI任务表(ai_task)的字段结构说明，反映biz_id字段从数值类型改为varchar(64)的重要变更
- 新增了system_id字段的详细说明，用于外部系统集成追踪
- 更新了相关索引设计和性能影响分析
- 补充了外部系统集成的架构说明
...
**最新更新**：AI任务表(ai_task)已完成重要字段结构调整，biz_id字段从数值类型升级为varchar(64)，并新增system_id字段以支持外部系统集成追踪功能。
...
- **新增**：字段迁移脚本：sql/20260713_bizId字段改为str.sql
...
- 支撑中心（support）
  ...
  - 说明：该模块负责系统基础能力与AI模型配置管理，AI模块仅读取模型配置。
...
- 核心业务（core）
  ...
  - 外部回调：ai_task_external_callback

- AI服务（ai）
  - 任务队列：ai_task（PENDING/PROCESSING/COMPLETED/FAILED/AI_UNAVAILABLE/SKIPPED）
    - **重要更新**：biz_id字段已升级为varchar(64)，支持更灵活的业务ID格式
    - **新增字段**：system_id用于外部系统集成追踪
...
- [ele-ai-tender-system/ele-ai-tender-core/sql/init.sql:18-272](file://ele-ai-tender-system/ele-ai-tender-core/sql/init.sql#L18-L272)
...
AI_TASK_EXTERNAL_CALLBACK {
bigint id PK
bigint task_id
string app_key
string callback_status
int retry_count
datetime last_callback_time
string error_msg
}
AI_TASK {
bigint id PK
...
- 外部回调
  - ai_task_external_callback：callback_status、task_id索引用于重试与结果推送。
...
### AI服务（ai）
- 任务队列
  - ai_task：**重要更新** - 字段结构已调整以支持外部系统集成
    - **biz_id字段升级**：从数值类型改为varchar(64)，支持更灵活的业务ID格式（如UUID、外部系统ID等）
    - **新增system_id字段**：bigint类型，用于标识调用系统的ID，实现外部系统集成追踪
...
**更新** AI任务表已完成重要的字段结构调整，增强了对外部系统集成的支持能力。

章节来源
- [ele-ai-tender-system/ele-ai-tender-ai/sql/init.sql:12-119](file://ele-ai-tender-system/ele-ai-tender-ai/sql/init.sql#L12-L119)
- [sql/20260713_bizId字段改为str.sql:1-6](file://sql/20260713_bizId字段改为str.sql#L1-L6)
- [ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/ai/AiTask.java:19-83](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/ai/AiTask.java#L19-L83)
...
- **新增**：字段迁移脚本
  - 20260713_bizId字段改为str.sql：执行ai_task表的biz_id字段类型升级和system_id字段添加操作
...
- **新增**：外部系统集成
  - ExternalAiTaskController提供外部API接口
  - ExternalAiTaskService处理外部请求并设置system_id
  - 通过appKey验证外部系统身份
...
**图表来源**
- [ele-ai-tender-system/ele-ai-tender-core/sql/init.sql:218-272](file://ele-ai-tender-system/ele-ai-tender-core/sql/init.sql#L218-L272)
- [ele-ai-tender-system/ele-ai-tender-ai/sql/init.sql:12-119](file://ele-ai-tender-system/ele-ai-tender-ai/sql/init.sql#L12-L119)
- [ele-ai-tender-system/ele-ai-tender-file/sql/init.sql:12-35](file://ele-ai-tender-system/ele-ai-tender-file/sql/init.sql#L12-L35)
- [ele-ai-tender-system/ele-ai-tender-support/sql/init.sql:328-390](file://ele-ai-tender-system/ele-ai-tender-support/sql/init.sql#L328-L390)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java:17-59](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java#L17-L59)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java:45-89](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java#L45-L89)
...
**重要更新**：本次架构变更显著增强了AI任务系统的外部集成能力。通过将biz_id字段升级为varchar(64)并新增system_id字段，系统现在能够支持更灵活的业务ID格式和完整的外部系统集成追踪。建议在后续演进中持续完善JSON字段的查询策略与大文本字段的归档方案，并结合慢查询日志优化热点索引，同时重点关注新字段带来的性能影响。
...
- 推荐顺序
  ...
  6) **新增**：字段迁移：sql/20260713_bizId字段改为str.sql（biz_id字段类型升级和system_id字段添加）
- 注意事项
  - 所有脚本均指向同一数据库ele_ai_tender，请勿重复执行导致幂等问题。
  - 如需重置自增ID，参考清理脚本注释中的ALTER TABLE示例。
  - **字段迁移注意事项**：在执行biz_id字段类型升级前，请确保备份数据并停止相关服务，以避免数据不一致。
