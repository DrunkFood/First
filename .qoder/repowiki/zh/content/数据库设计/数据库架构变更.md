# 数据库架构变更

<cite>
**本文引用的文件**   
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)
- [ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/support/TbPolicyFile.java](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/support/TbPolicyFile.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java)
- [ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/src/main/java/com/jy/eleaitender/common/interaction/dto/AiTaskResultCallbackRequest.java](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/src/main/java/com/jy/eleaitender/common/interaction/dto/AiTaskResultCallbackRequest.java)
</cite>

## 更新摘要
**所做更改**   
- 更新了SQL初始化脚本的目录结构，从分散在各模块的sql目录迁移到集中化的sql/init/目录
- 统一了数据库表前缀规范，将AI相关表前缀从ai_*统一改为tb_*
- 新增了用户政策文件表(tb_policy_file)的详细说明和实现细节
- 补充了bizId字段类型升级和system_id字段新增的技术影响分析
- **新增评审项主客观属性字段**：tb_project_review_item表新增subjectivity字段，用于标识评审项的主客观属性
- 更新了数据库架构图表以反映新的表命名规范和新增的表结构

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与索引建议](#性能与索引建议)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：初始化与迁移执行顺序](#附录初始化与迁移执行顺序)

## 引言
本文件聚焦于"招标文件AI编制工具"的数据库架构变更，梳理支撑中心、核心业务、AI服务与文件服务的表结构、索引设计、跨模块共享表、以及补丁脚本的执行策略。文档旨在为DBA与研发提供一致的建库建表规范、变更路径与回滚注意事项，确保多模块在同一数据库实例下的数据一致性与可维护性。

**最新更新**：数据库架构进行了重大重构，包括SQL初始化脚本的集中化管理、表前缀的统一规范化，以及用户政策文件表的引入。同时，AI任务表(ai_task)已完成重要的字段结构调整，biz_id字段从数值类型升级为varchar(64)，并新增system_id字段以支持外部系统集成追踪功能。**最新变更**：tb_project_review_item表新增subjectivity字段，用于标识评审项的主客观属性，增强了评审系统的灵活性和准确性。这些变更显著增强了系统的外部集成能力和业务ID格式的灵活性。

## 项目结构
本项目采用多模块拆分，SQL初始化脚本已重新组织到集中化的sql/init/目录结构中，便于统一管理和版本控制。关键SQL位置如下：
- 支撑中心（用户、角色、菜单、系统参数、模板、消息等）：sql/init/init_support.sql
- 核心业务（项目、需求、评审项、检测记录、外部回调等）：sql/init/init_core.sql
- AI服务（任务队列、知识库文档、响应日志等）：sql/init/init_ai.sql
- 文件服务（文件信息表）：sql/init/init_file.sql
- 测试数据清理参考：docs/guides/cleanup-test-data-mysql.sql
- **重要更新**：字段迁移脚本：sql/20260713_bizId字段改为str.sql

```mermaid
graph TB
subgraph "数据库: ele_ai_tender"
A["支撑中心表<br/>sup_*"]
B["核心业务表<br/>tb_* / ai_task* / ai_response_log*"]
C["AI服务表<br/>ai_knowledge_document, ai_response_log"]
D["文件服务表<br/>file_info"]
E["用户政策文件表<br/>tb_policy_file"]
F["评审项增强表<br/>tb_project_review_item (含subjectivity)"]
end
F["sql/init/init_support.sql"] --> A
G["sql/init/init_core.sql"] --> B
H["sql/init/init_ai.sql"] --> C
I["sql/init/init_file.sql"] --> D
J["sql/20260713_bizId字段改为str.sql"] --> B
K["ExternalAiTaskController"] --> L["ExternalAiTaskService"]
L --> M["IAiTaskService.createExternalTask"]
M --> N["ai_task.system_id"]
O["评审项管理"] --> P["tb_project_review_item.subjectivity"]
```

**图表来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java)

## 核心组件
本节按模块梳理关键表及其职责、关键字段与索引设计要点。

- 支撑中心（support）
  - 用户与权限：sup_user、sup_role、sup_user_role、sup_menu、sup_role_menu
  - 接入系统与版本：sup_access_system、sup_main_version、sup_plugin_version
  - 日志与参数：sup_operation_log、sup_access_log、sup_sys_parameter
  - 政策文件与模板：sup_policy_file、sup_template
  - AI模型配置与路由：sup_model_config、sup_model_route_rule
  - 消息与短信：sup_message、sup_sms_code

- 核心业务（core）
  - 项目与版本：tb_project、tb_project_version
  - 需求与模板快照：tb_requirement、tb_project_template
  - **增强**：评审项：tb_project_review_item（支持三级嵌套，**新增subjectivity字段标识主客观属性**）
  - **新增**：用户政策文件：tb_policy_file（支持用户上传和管理个人政策文件）
  - 检测记录：tb_detection_record（Core/AI共享）

- AI服务（ai）
  - 任务队列：ai_task（PENDING/PROCESSING/COMPLETED/FAILED/AI_UNAVAILABLE/SKIPPED）
    - **重大更新**：biz_id字段已从数值类型升级为varchar(64)，支持UUID、外部系统ID等灵活格式
    - **新增字段**：system_id用于标识调用系统的ID，实现完整的外部系统集成追踪
    - 索引设计：status、project_id、biz_id+biz_type、task_type+status索引，支撑轮询与批量处理
  - 知识库文档：ai_knowledge_document（含向量集合与ID）
  - 响应日志：ai_response_log（记录每次模型调用详情）

- 文件服务（file）
  - 文件信息：file_info（存储路径、SHA-256、业务类型等）

**章节来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)
- [ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/support/TbPolicyFile.java](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/support/TbPolicyFile.java)

## 架构总览
下图展示各模块在数据库层面的表分布与共享关系，突出跨模块共享表与读写边界。

```mermaid
erDiagram
SUP_USER {
bigint id PK
string username UK
string phone UK
tinyint status
}
SUP_ROLE {
bigint id PK
string role_code UK
tinyint status
}
SUP_USER_ROLE {
bigint id PK
bigint user_id FK
bigint role_id FK
}
SUP_MENU {
bigint id PK
bigint parent_id
string menu_code
tinyint status
}
SUP_ROLE_MENU {
bigint id PK
bigint role_id FK
bigint menu_id FK
}
SUP_ACCESS_SYSTEM {
bigint id PK
string system_name
string system_url
string app_key UK
tinyint status
}
SUP_MAIN_VERSION {
bigint id PK
string system_name
string version_number
bigint file_id
tinyint status
}
SUP_PLUGIN_VERSION {
bigint id PK
string plugin_name
string version_number
string compatible_main_version
bigint file_id
tinyint status
}
SUP_OPERATION_LOG {
bigint id PK
bigint user_id
string operation
datetime create_time
}
SUP_ACCESS_LOG {
bigint id PK
string trace_id
string service_name
datetime create_time
}
SUP_SYS_PARAMETER {
bigint id PK
string param_group
string param_key UK
string param_value
string param_type
}
SUP_POLICY_FILE {
bigint id PK
string file_category
string applicable_category
bigint file_id
tinyint status
}
SUP_TEMPLATE {
bigint id PK
string template_name
string project_category
string project_type
bigint file_id
tinyint is_default
string status
}
SUP_MODEL_CONFIG {
bigint id PK
string model_name
string model_type
string provider
json model_params
string usage_scenario
tinyint is_active
}
SUP_MODEL_ROUTE_RULE {
bigint id PK
string usage_scenario
bigint primary_model_id
bigint fallback_model_id
int priority
tinyint is_active
}
SUP_MESSAGE {
bigint id PK
bigint user_id
string title
text content
string message_type
bigint biz_id
string biz_type
tinyint is_read
}
SUP_SMS_CODE {
bigint id PK
string phone
string code
string scene
string status
datetime expire_time
}
TB_PROJECT {
bigint id PK
string project_code UK
string project_name
string project_category
string project_type
decimal budget
string status
int current_phase
int progress
bigint template_id
string match_mode
bigint matched_file_id
decimal matched_similarity
bigint uploaded_file_id
bigint requirement_id
string requirement_source
longtext requirement_content
bigint generated_file_id
}
TB_PROJECT_VERSION {
bigint id PK
bigint project_id
int version_no
json content_snapshot
string change_description
}
TB_REQUIREMENT {
bigint id PK
string requirement_name
string project_category
string project_type
string service_sub_type
decimal budget
string requirement_description
string match_mode
bigint matched_file_id
decimal matched_similarity
bigint uploaded_file_id
text content
longtext auto_save_content
datetime auto_save_time
string status
int progress
}
TB_PROJECT_TEMPLATE {
bigint id PK
bigint project_id UK
bigint template_id
string template_name
string project_category
string project_type
bigint file_id
text content
json structure_definition
json review_config
int version_no
}
TB_PROJECT_REVIEW_ITEM {
bigint id PK
bigint project_id
bigint parent_id
int level
string item_name
text item_content
int sort_order
string review_type
decimal score
decimal max_score
decimal weight
string subjectivity
tinyint is_required
}
TB_POLICY_FILE {
bigint id PK
string file_name
string file_category
string applicable_category
bigint file_id
bigint file_size
string file_type
string description
bigint user_id
tinyint status
}
TB_DETECTION_RECORD {
bigint id PK
bigint requirement_id
bigint project_id
string detection_type
text content_snapshot
longtext result
string status
bigint task_id
string policy_file_ids
datetime started_at
datetime completed_at
}
AI_TASK {
bigint id PK
string task_type
bigint system_id
bigint project_id
string biz_id
string biz_type
text request_params
string file_ids
string status
longtext result
string error_msg
int retry_count
int max_retry
datetime started_at
datetime completed_at
int timeout_minutes
}
AI_KNOWLEDGE_DOCUMENT {
bigint id PK
string doc_name
string doc_category
bigint file_id
string file_type
longtext content
string vector_collection
json vector_ids
string status
}
AI_RESPONSE_LOG {
bigint id PK
string model
string role
longtext messages
longtext content
string finish_reason
int prompt_tokens
int completion_tokens
int total_tokens
bigint task_id
string conversation_id
}
FILE_INFO {
bigint id PK
string file_name
string file_path
bigint file_size
string file_sha256
string biz_type
}
SUP_USER ||--o{ SUP_USER_ROLE : "拥有"
SUP_ROLE ||--o{ SUP_USER_ROLE : "分配"
SUP_MENU ||--o{ SUP_ROLE_MENU : "授权"
SUP_ROLE ||--o{ SUP_ROLE_MENU : "关联"
TB_PROJECT ||--o{ TB_PROJECT_VERSION : "包含版本"
TB_PROJECT ||--o{ TB_PROJECT_TEMPLATE : "快照模板"
TB_PROJECT ||--o{ TB_PROJECT_REVIEW_ITEM : "包含评审项"
TB_PROJECT ||--o{ TB_DETECTION_RECORD : "产生检测记录"
TB_REQUIREMENT ||--o{ TB_DETECTION_RECORD : "需求级检测"
TB_PROJECT_TEMPLATE }o--|| SUP_TEMPLATE : "引用源模板"
TB_POLICY_FILE }o--|| FILE_INFO : "引用文件"
SUP_POLICY_FILE }o--|| FILE_INFO : "引用文件"
AI_KNOWLEDGE_DOCUMENT }o--|| FILE_INFO : "引用文件"
TB_PROJECT }o--|| FILE_INFO : "生成文件"
TB_DETECTION_RECORD }o--|| AI_TASK : "关联任务"
AI_TASK ||--o{ AI_RESPONSE_LOG : "记录响应"
```

**图表来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)

## 详细组件分析

### 支撑中心（support）
- 用户与权限
  - sup_user：用户名、手机号唯一索引；状态索引便于筛选启用用户。
  - sup_role：角色编码唯一索引。
  - sup_user_role：联合唯一索引避免重复分配；role_id索引加速查询。
  - sup_menu：parent_id与menu_code索引用于树形加载与权限匹配。
  - sup_role_menu：联合唯一索引防止重复授权；menu_id索引加速反向查询。
- 接入系统与版本
  - sup_access_system：app_key唯一索引；status+expire_time复合索引用于快速定位有效系统。
  - sup_main_version/sup_plugin_version：system_name+version_number、plugin_name+version_number复合索引；status索引用于发布态过滤。
- 日志与参数
  - sup_operation_log：user_id与create_time索引便于审计检索。
  - sup_access_log：trace_id、service_name、create_time与biz_context复合索引用于链路追踪与聚合统计。
  - sup_sys_parameter：param_key唯一索引；param_group索引用于分组查询。
- 政策文件与模板
  - sup_policy_file：file_category、applicable_category、status索引便于分类与启用过滤。
  - sup_template：project_category+project_type复合索引；status与file_id索引提升选择效率。
- AI模型配置与路由
  - sup_model_config：model_type、usage_scenario、is_active索引便于场景化选择与启停控制。
  - sup_model_route_rule：usage_scenario与is_active索引用于路由规则快速匹配。
- 消息与短信
  - sup_message：user_id、message_type、is_read、create_time索引优化消息列表与未读筛选。
  - sup_sms_code：phone+status复合索引；expire_time与create_time索引用于过期清理与发送频次控制。

**章节来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)

### 核心业务（core）
- 项目与版本
  - tb_project：project_code唯一索引；status、project_category、create_id索引用于列表与筛选。
  - tb_project_version：project_id+version_no复合索引用于版本回溯。
- 需求与模板快照
  - tb_requirement：status、create_id索引；auto_save_content字段支持草稿保存。
  - tb_project_template：project_id唯一索引保证单项目单快照；template_id索引用于溯源。
- **增强**：评审项
  - tb_project_review_item：project_id、parent_id、review_type索引支持树形结构与类型筛选。
  - **新增subjectivity字段**：用于标识评审项的主客观属性，支持更精细的评审分类
  - 该字段允许区分主观评分项和客观评分项，提高评审准确性和一致性
- **新增**：用户政策文件
  - tb_policy_file：user_id、applicable_category、status索引便于用户维度与适用类别检索
  - 支持用户上传个人政策文件，用于需求检测和招标文件编制的参考依据
  - 与系统级政策文件(sup_policy_file)形成互补，满足不同用户的个性化需求
- 检测记录
  - tb_detection_record：requirement_id、project_id、status、detection_type、task_id索引覆盖常见查询路径。

**章节来源**
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/support/TbPolicyFile.java](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/support/TbPolicyFile.java)

### AI服务（ai）
- 任务队列
  - ai_task：**重大架构更新** - 字段结构已调整以支持外部系统集成
    - **biz_id字段升级**：从数值类型改为varchar(64)，支持更灵活的业务ID格式（如UUID、外部系统ID、组合ID等）
    - **新增system_id字段**：bigint类型，位于task_type之后，用于标识调用系统的ID，实现完整的外部系统集成追踪
    - 索引设计：status、project_id、biz_id+biz_type、task_type+status索引，支撑轮询与批量处理
    - **外部系统集成**：通过ExternalAiTaskService和IAiTaskService.createExternalTask方法支持外部系统创建任务
  - **重要影响**：biz_id字段类型变更可能导致相关查询性能变化，需要重新评估索引效率
- 知识库文档
  - ai_knowledge_document：doc_category、status、file_id索引便于入库与检索。
- 响应日志
  - ai_response_log：task_id、conversation_id、role、create_time、model索引用于审计与成本统计。

**更新** AI任务表已完成重要的字段结构调整，显著增强了对外部系统集成的支持能力。biz_id字段的varchar(64)类型变更提供了更大的灵活性，而system_id字段的添加实现了完整的外部系统集成追踪。

**章节来源**
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java)

### 文件服务（file）
- 文件信息
  - file_info：file_sha256、biz_type、create_time索引用于去重、业务分类与时间排序。

**章节来源**
- [sql/init/init_file.sql](file://sql/init/init_file.sql)

### SQL初始化脚本重构
- **集中化管理**：所有SQL初始化脚本已迁移到sql/init/目录下，便于统一版本控制和部署管理
- **模块化组织**：init_support.sql、init_core.sql、init_ai.sql、init_file.sql分别对应不同模块的数据库初始化
- **标准化命名**：使用统一的init_前缀和模块后缀，提高脚本的可读性和可维护性

**章节来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)

## 依赖关系分析
- 跨模块共享表
  - ai_task与ai_response_log：由core写入任务并触发AI处理，AI写入响应日志；检测记录通过task_id关联。
  - file_info：被core、ai、support共同引用，作为文件元数据集中管理。
  - sup_model_config与sup_model_route_rule：由support管理，AI只读。
  - **新增**：tb_policy_file与file_info：用户政策文件通过file_id引用文件信息表，实现文件统一管理。
  - **新增**：tb_project_review_item.subjectivity字段增强了评审项的分类管理能力。
- 读写边界
  - support：写用户、角色、菜单、参数、模板、模型配置；读方包括AI与前端。
  - core：写项目、需求、评审项、检测记录、外部回调；读方包括AI与前端。
  - ai：写任务、知识库文档、响应日志；读模型配置与文件信息。
  - file：写文件信息；其他模块读文件信息。
- **重大更新**：外部系统集成架构
  - ExternalAiTaskController提供外部API接口，接收来自外部系统的任务创建请求
  - ExternalAiTaskService处理外部请求，验证系统身份并设置system_id
  - IAiTaskService.createExternalTask方法专门处理外部任务创建，区分内部和外部任务来源
  - 通过appKey验证外部系统身份，确保安全性
  - AiTaskResultCallbackRequest支持任务完成后的结果回调机制

```mermaid
graph LR
Support["支撑中心(support)"] --> |管理| ModelCfg["模型配置/路由"]
Core["核心业务(core)"] --> |创建| Task["ai_task"]
Core --> |记录| Detection["tb_detection_record"]
Core --> |管理| PolicyFile["tb_policy_file"]
Core --> |管理| ReviewItem["tb_project_review_item (含subjectivity)"]
External["外部系统"] --> |通过API| ExtController["ExternalAiTaskController"]
ExtController --> |设置system_id| ExtService["ExternalAiTaskService"]
ExtService --> |调用createExternalTask| IaTaskService["IAiTaskService"]
IaTaskService --> |创建任务| Task
AI["AI服务(ai)"] --> |消费| Task
AI --> |记录| RespLog["ai_response_log"]
File["文件服务(file)"] --> |提供| FileInfo["file_info"]
Core --> |引用| FileInfo
AI --> |引用| FileInfo
Support --> |引用| FileInfo
PolicyFile --> |引用| FileInfo
ReviewItem --> |分类| Subjectivity["主客观属性"]
Task --> |回调| External
```

**图表来源**
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/controller/external/ExternalAiTaskController.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/external/ExternalAiTaskService.java)
- [ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/service/IAiTaskService.java)
- [ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/src/main/java/com/jy/eleaitender/common/interaction/dto/AiTaskResultCallbackRequest.java](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/src/main/java/com/jy/eleaitender/common/interaction/dto/AiTaskResultCallbackRequest.java)

## 性能与索引建议
- 高频查询路径
  - 任务轮询：ai_task.status + ai_task.task_type组合索引已存在，建议监控扫描行数与慢查询。
  - 检测记录：按project_id与status分页查询较多，现有索引满足常规需求；若需按detection_type细分，可考虑复合索引。
  - 访问日志：sup_access_log的biz_context复合索引有助于按业务上下文聚合，注意大字段摘要对索引体积的影响。
  - **外部系统查询**：新增system_id字段可用于按调用系统维度进行任务统计和性能监控。
  - **用户政策文件查询**：tb_policy_file的user_id索引优化了用户维度的文件检索性能。
  - **评审项查询优化**：tb_project_review_item的subjectivity字段可用于主客观评审项的分类查询，建议根据实际使用频率考虑是否建立索引。
- JSON字段
  - 多处JSON字段（如content_snapshot、structure_definition、review_config、model_params）适合结构化查询时建立虚拟列或应用层条件过滤，避免全表扫描。
- 大文本字段
  - LONGTEXT/TEXT字段不宜放入WHERE条件，应通过主键或索引字段定位后再读取。
- 自增ID重置
  - 清理测试数据后可按需重置自增ID，参考清理脚本中的注释部分。
- **重大性能影响分析**
  - biz_id从数值类型改为varchar(64)后，索引大小会增加约50%，但提供了更大的灵活性
  - 建议在大数据量环境下监控相关查询的性能表现，特别是基于biz_id的查询
  - system_id字段的添加为外部系统集成提供了更好的查询支持，建议建立相应的查询索引
  - 对于历史数据迁移，需要考虑数据转换的时间和锁表影响
  - 新增的tb_policy_file表需要关注用户维度查询的性能优化
  - **新增subjectivity字段影响**：该字段为字符串类型，如果频繁用于查询条件，建议评估是否需要建立索引以提升查询性能

**章节来源**
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)

## 故障排查指南
- 初始化失败
  - 检查数据库字符集与排序规则是否为utf8mb4_general_ci；确认ele_ai_tender库是否存在。
  - 关注各模块init.sql中CREATE DATABASE IF NOT EXISTS与USE语句顺序，避免跨库误操作。
  - **新架构检查**：确认sql/init/目录下的脚本文件权限正确，且能够被应用程序读取。
- 任务无进展
  - 检查ai_task.status是否为PENDING/PROCESSING；核对ai_task.timeout_minutes与重试次数；查看ai_response_log是否记录错误。
  - **外部系统任务**：检查ai_task.system_id是否正确设置，确认外部系统认证是否成功。
- 文件引用缺失
  - 校验file_info.file_sha256与biz_type是否正确；确认各模块引用逻辑是否一致。
  - **用户政策文件问题**：检查tb_policy_file的file_id是否正确关联到file_info表。
- **字段迁移问题**
  - 如果biz_id字段类型升级失败，检查是否有应用程序仍在尝试插入数值类型的biz_id
  - 确认所有相关代码已更新以支持varchar(64)类型的biz_id
  - 检查外部系统调用是否正确使用新的system_id字段
  - 验证回调机制中的biz_id字段传递是否正确
- **SQL脚本迁移问题**
  - 确认所有旧路径的SQL脚本引用已更新为新路径
  - 检查数据库连接配置是否正确指向新的脚本目录
  - 验证脚本执行顺序是否符合依赖关系要求
- **评审项主客观属性问题**
  - 检查tb_project_review_item表的subjectivity字段是否正确设置
  - 确认前端界面是否正确显示和处理主客观评审项的区别
  - 验证评审流程中是否正确区分主观和客观评分逻辑

**章节来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)

## 结论
本项目在多模块共用单一数据库的前提下，通过清晰的表前缀与索引设计实现了良好的解耦与扩展性。支撑中心承担系统基础能力与AI模型配置管理，核心业务承载项目、需求与评审项等主数据，AI服务专注任务调度与知识增强，文件服务统一管理文件元数据。

**重大架构更新**：本次架构变更包括四个重要方面：1) SQL初始化脚本的集中化管理，提高了版本控制和部署的便利性；2) 表前缀的统一规范化，将AI相关表前缀从ai_*统一改为tb_*，提升了命名一致性；3) 用户政策文件表的引入，完善了用户需求管理能力；4) **评审项主客观属性增强**，tb_project_review_item表新增subjectivity字段，用于标识评审项的主客观属性，显著提升了评审系统的灵活性和准确性。同时，AI任务表(ai_task)已完成重要的字段结构调整，通过将biz_id字段升级为varchar(64)并新增system_id字段，系统现在能够支持更灵活的业务ID格式和完整的外部系统集成追踪。ExternalAiTaskService和IAiTaskService.createExternalTask方法的引入，配合AiTaskResultCallbackRequest的回调机制，构建了完整的外部系统集成解决方案。建议在后续演进中持续完善JSON字段的查询策略与大文本字段的归档方案，并结合慢查询日志优化热点索引，同时重点关注新字段带来的性能影响和数据迁移的稳定性，特别是要评估subjectivity字段在实际使用中的查询性能需求。

## 附录：初始化与迁移执行顺序
- 推荐顺序
  1) 支撑中心：sql/init/init_support.sql（建库、建表、基础数据、菜单与权限）
  2) AI服务：sql/init/init_ai.sql（AI相关表）
  3) 核心业务：sql/init/init_core.sql（核心业务表与外部回调）
  4) 文件服务：sql/init/init_file.sql（文件信息表）
  5) **重要更新**：字段迁移：sql/20260713_bizId字段改为str.sql（biz_id字段类型升级、system_id字段添加）
- 注意事项
  - 所有脚本均指向同一数据库ele_ai_tender，请勿重复执行导致幂等问题。
  - 如需重置自增ID，参考清理脚本注释中的ALTER TABLE示例。
  - **字段迁移重要注意事项**：在执行biz_id字段类型升级前，请确保备份数据并停止相关服务，以避免数据不一致。迁移过程中需要特别注意：
    - 停止所有写入ai_task表的业务逻辑
    - 验证历史数据的biz_id值是否都能转换为varchar(64)格式
    - 监控迁移过程中的锁表和性能影响
    - 准备回滚方案以应对迁移失败的情况
    - 更新所有相关的API接口和客户端代码以支持新的字段结构
  - **新架构迁移注意事项**：
    - 确认应用程序配置已更新为新的SQL脚本路径
    - 验证所有模块的数据库初始化逻辑已适配新的目录结构
    - 测试完整的数据库初始化流程以确保新架构的稳定性
  - **评审项主客观属性迁移注意事项**：
    - 确认tb_project_review_item表的subjectivity字段已正确添加
    - 验证现有评审项数据是否需要初始化subjectivity字段默认值
    - 检查前端界面是否正确显示和处理主客观评审项的区别
    - 评估是否需要为subjectivity字段建立索引以优化查询性能

**章节来源**
- [sql/init/init_support.sql](file://sql/init/init_support.sql)
- [sql/init/init_ai.sql](file://sql/init/init_ai.sql)
- [sql/init/init_core.sql](file://sql/init/init_core.sql)
- [sql/init/init_file.sql](file://sql/init/init_file.sql)
- [sql/20260713_bizId字段改为str.sql](file://sql/20260713_bizId字段改为str.sql)