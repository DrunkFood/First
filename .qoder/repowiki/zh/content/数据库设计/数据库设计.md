...
- AI任务实体 AiTask：对应 ai_task，作为 Core 与 AI 之间的异步任务载体。
...
- [ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/ai/AiTask.java:1-77](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/entity/ai/AiTask.java#L1-L77)
...
### AI服务（ai）关键表
- AI任务队列表：ai_task（状态机、重试、超时、结果同步标记）
...
Core->>DB : 插入 ai_task(status=PENDING)
...
```mermaid
flowchart TD
Start(["开始"]) --> CreateTask["创建 ai_task(PENDING)"]
...
## 结论
本数据库设计围绕“支撑中心-编制中心-AI服务-文件服务”四大领域展开，采用统一的审计与并发控制机制，并通过索引与缓存提升性能。Core 与 AI 之间以 ai_task 为纽带，确保异步任务的可观测与可重试。整体架构清晰、扩展性强，适合在分布式环境下稳定运行。
