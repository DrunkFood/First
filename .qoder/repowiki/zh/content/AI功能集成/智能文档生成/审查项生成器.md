...
## 更新摘要
**变更内容**   
- 新增详细的配置选项支持，包括项目信息、评审方法、要求内容和JSON格式的评审配置
- 扩展了四种评审类型（合规性、技术、信用和商业）的完整配置能力
- 增强了评分模式配置和手动评审项配置功能
- 优化了提示词模板以支持更精确的配置驱动生成
...
**更新** 新增了详细的配置选项支持，包括项目信息、评审方法、要求内容和JSON格式的评审配置，支持完整的四种评审类型配置。
...
subgraph "通用层"
E["ReviewType / ScoreMode / SubjectivityType<br/>评审类型与评分模式"]
F["ReviewConfig / ReviewTypeConfig / TemplateReviewItemConfig<br/>配置与模板项"]
G["ReviewItemGenerateParams<br/>任务参数"]
end
subgraph "核心服务"
H["AiTaskResultSyncHandler<br/>解析与落库"]
I["DocumentDataAssembler<br/>汇总表构建"]
end
subgraph "前端"
J["PhaseReviewItem.vue<br/>树视图与计分"]
K["review.ts / template.ts<br/>类型与默认配置"]
end
A --> B
A --> C
A --> D
...
- [TemplateReviewItemConfig.java:12-32](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/TemplateReviewItemConfig.java#L12-32)
- [ReviewItemGenerateParams.java:1-26](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/ai/ReviewItemGenerateParams.java#L1-26)
...
- 审查项生成器（ReviewItemGenerator）
  ...
  - 关键点：支持SCORE与WEIGHT两种评分模式；兼容多种AI输出格式；自动将分类对象转换为统一树结构；支持详细的配置选项。
- 提示词模板（SystemPromptTemplates / UserPromptTemplates / PromptBuilder）
  - 职责：约束AI输出的结构与评分规则，确保符合招投标法规与行业惯例。
  - 关键点：明确"符合性审查不计入总分""叶子节点合计必须等于目标分或权重合计=100%"等硬性规则；支持配置驱动的提示词构建。
- 配置与枚举（ReviewConfig / ReviewTypeConfig / TemplateReviewItemConfig / ReviewType / ScoreMode / SubjectivityType）
  - 职责：定义评审类型、是否启用、是否由AI生成标准、是否区分主客观、手动模板项、评分模式等。
  - 关键点：向后兼容null值，默认回退SCORE模式；支持模板级manualItems作为AI未生成时的兜底；支持四种完整评审类型配置。
- 任务参数（ReviewItemGenerateParams）
  - 职责：封装评审项生成的所有必要参数，包括项目信息、评审方法和配置。
  - 关键点：支持JSON格式的评审配置，包含项目基本信息和详细的评审类型设置。
...
- [ReviewType.java:14-36](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/enums/ReviewType.java#L14-L36)
- [ScoreMode.java:8-29](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/enums/ScoreMode.java#L8-L29)
- [SubjectivityType.java:1-27](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/enums/SubjectivityType.java#L1-27)
- [ReviewItemGenerateParams.java:1-26](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/ai/ReviewItemGenerateParams.java#L1-26)
- [AiTaskResultSyncHandler.java:494-522](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/scheduler/AiTaskResultSyncHandler.java#L494-L522)
- [DocumentDataAssembler.java:106-121](file://ele-ai-tender-system/ele-ai-tender-core/src/main/java/com/jy/eleaitender/core/engine/DocumentDataAssembler.java#L106-L121)
- [PhaseReviewItem.vue:56-84](file://ele-ai-tender-frontend/src/views/project/phases/PhaseReviewItem.vue#L56-L84)
...
**更新** 现在支持更详细的配置选项，包括项目信息、评审方法、要求内容和JSON格式的评审配置。

```mermaid
sequenceDiagram
participant UI as "前端界面"
participant Gen as "ReviewItemGenerator"
participant Params as "ReviewItemGenerateParams"
...
- [ReviewItemGenerator.java:74-154](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/processor/generator/ReviewItemGenerator.java#L74-L154)
- [ReviewItemGenerateParams.java:1-26](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/ai/ReviewItemGenerateParams.java#L1-26)
- [ReviewConfig.java:53-60](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/ReviewConfig.java#L53-60)
...
**更新** 现在支持完整的四种评审类型配置，每种类型都有详细的配置选项。
...
+of(type, enabled, generateStandard, distinguishSubjectivity) ReviewTypeConfig
}
...
ReviewItemGenerateParams --> ReviewConfig : "使用"
```
...
**更新** 现在支持更详细的配置选项，包括项目信息、评审方法、要求内容和JSON格式的评审配置，使AI能够生成更精确的评审标准。
...
**更新** 现在支持更详细的配置选项，包括项目信息、评审方法、要求内容和JSON格式的评审配置，使层次化结构更加灵活。
...
**更新** 现在支持更详细的配置选项，包括项目信息、评审方法、要求内容和JSON格式的评审配置，使评分模型更加灵活和精确。
...
**更新** 现在支持更详细的配置选项，包括项目信息、评审方法、要求内容和JSON格式的评审配置，使可视化展示和报告生成更加精确。
...
**更新** 新增了对ReviewItemGenerateParams的详细依赖，支持更复杂的配置选项。
...
Gen --> Params["ReviewItemGenerateParams"]
...
**更新** 新增的详细配置选项提高了系统的可扩展性，支持更灵活的评审类型和评分模式配置。
...
## 结论
审查项生成器通过"配置驱动+提示词约束+JSON归一化与修复"的组合，实现了从招标文件到标准化评审标准树的自动化生成。其支持分值与权重两种评分模式，兼容多种AI输出格式，并提供完善的可视化与报告能力。

**更新** 现在支持更详细的配置选项，包括项目信息、评审方法、要求内容和JSON格式的评审配置，支持完整的四种评审类型配置，进一步提升了系统的灵活性和实用性。建议在后续迭代中持续优化提示词质量、增强修复策略与监控指标，以提升稳定性与可用性。
