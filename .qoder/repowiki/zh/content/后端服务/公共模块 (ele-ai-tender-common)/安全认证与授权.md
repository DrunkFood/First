...
## 更新摘要
**变更内容**   
- ExternalSignatureInterceptor和FileSignatureInterceptor现在使用@Component注解进行自动Spring组件扫描，消除了配置类中的手动Bean注册
- 这代表了向更惯用的Spring Boot实践的转变，同时保持相同的功能行为
- 简化了拦截器的注册方式，提高了代码的可维护性
...
Wf["WebConfig(file)<br/>注册过滤器+拦截器"]
end
F --> Ctx
F --> User
F --> Jwt
...
- [WebConfig.java（file）:56-59](file://ele-ai-tender-system/ele-ai-tender-file/src/main/java/com/jy/eleaitender/file/config/WebConfig.java#L56-L59)

## 核心组件
- JwtAuthenticationFilter：统一鉴权入口，负责从请求中提取Token、校验签名、按类型进行Redis一致性校验，并将登录用户信息写入线程上下文。
...
## 架构总览
下图展示了从HTTP请求进入，到过滤器校验、上下文填充、业务方法执行、以及登出清理的整体流程，包括组件化的签名验证拦截器。
...
CheckExternal --> |一致| BuildExt["构建外部用户"]
CheckExternal --> |不一致| Next
BuildSvc --> SetCtx["写入SecurityContextHolder"]
LoadRoles --> SetCtx
BuildExt --> SetCtx
SetCtx --> Next
Next --> End(["请求结束清理上下文"])
...
- Web配置
  - support与ai模块均注册JwtAuthenticationFilter，覆盖/api/*路径；file模块提供混合认证模式
- **更新**：签名拦截器依赖
...
- [FileSignatureInterceptor.java:62-85](file://ele-ai-tender-system/ele-ai-tender-file/src/main/java/com/jy/eleaitender/file/config/FileSignatureInterceptor.java#L62-L85)

## 结论
本模块以JWT为核心，结合Redis实现无状态但可控的认证体系，并通过AOP与注解简化鉴权开发。RSA公钥加密传输与密钥轮换保障敏感数据安全，角色与权限缓存提升性能与可扩展性。**更新的组件化注册方式使ExternalSignatureInterceptor和FileSignatureInterceptor通过@Component注解自动注册，简化了配置管理，提高了代码的可维护性**。新增的签名验证拦截器提供了灵活的外部系统集成方案，JSON转义安全增强提升了系统的整体安全性，智能会话清理逻辑避免了上下文污染问题。建议在部署中严格配置密钥、开启HTTPS、完善监控与审计，持续优化用户体验与安全基线。
