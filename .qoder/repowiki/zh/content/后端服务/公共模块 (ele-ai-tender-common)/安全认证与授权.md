...
## 更新摘要
**变更内容**   
- **重要更新**：优化了用户注册流程，自动设置最后登录时间字段，提升用户体验和系统一致性
- **重要更新**：改进了外部系统用户验证逻辑，增强了令牌过期解析机制的健壮性
...
subgraph "用户注册流程优化"
RegFlow["用户注册流程<br/>自动设置最后登录时间"]
AutoFill["自动填充机制<br/>lastLoginTime字段"]
end
subgraph "支撑服务"
...
## 核心组件
- JwtAuthenticationFilter：统一鉴权入口，负责从请求中提取Token、校验签名、按类型进行Redis一致性校验，并将登录用户信息写入线程上下文。**已更新**：完整支持EXTERNAL令牌类型的完整验证流程，新增systemId字段处理和增强的外部令牌验证逻辑，改进了令牌过期解析机制。
...
- JwtUtil：JWT签发、解析、校验、过期判断等工具；支持内部用户、外部系统、服务间调用三类Token。**重要更新**：增强了外部令牌生成，支持systemId、externalUserId、externalUserName等新字段，改进了令牌过期解析机制。
...
- **重要更新**：用户注册流程优化，自动设置最后登录时间字段，提升用户体验。
...
## 架构总览
下图展示了从HTTP请求进入，到过滤器校验、上下文填充、业务方法执行、以及登出清理的整体流程，包括组件化的签名验证拦截器、增强的EXTERNAL令牌支持和多租户系统标识处理，以及优化的用户注册流程。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant RegService as "用户注册服务"
...
Note over Client,Controller : 用户注册流程优化
Client->>RegService : "POST /api/auth/register"
RegService->>RegService : "创建新用户"
RegService->>RegService : "自动设置最后登录时间"
RegService-->>Client : "返回注册成功响应"
...
- 功能要点
  ...
  - **重要更新**：完整支持EXTERNAL令牌类型的验证流程，包括jti会话键和兼容旧键，新增systemId字段处理，改进了令牌过期解析机制
- 关键行为
  ...
  - **重要更新**：增强了外部令牌验证逻辑，支持更完善的Redis会话管理和角色加载，改进了令牌过期解析的健壮性
...
### 用户注册流程优化（重要更新）
- **重要更新**：优化了用户注册流程，自动设置最后登录时间字段
- 用户注册时自动填充lastLoginTime字段，提升用户体验
- 减少了手动设置最后登录时间的复杂性
- 确保新用户注册后立即拥有完整的用户信息
- 提升了系统的一致性和自动化程度
...
```mermaid
flowchart TD
RegisterRequest["用户注册请求"] --> CreateUser["创建用户对象"]
CreateUser --> AutoFill["自动填充字段"]
AutoFill --> SetLastLogin["设置最后登录时间"]
SetLastLogin --> SaveUser["保存用户信息"]
SaveUser --> ReturnSuccess["返回注册成功"]
```
...
AddClaims --> Expiry["expiry: 过期时间(改进解析)"]
...
- **重要更新**：增强了EXTERNAL令牌的验证逻辑，支持systemId和外部用户信息，改进了令牌过期解析
...
- [FileSignatureInterceptor.java:119-124](file://ele-ai-tender-system/ele-ai-tender-file/src/main/java/com/jy/eleaitender/file/config/FileSignatureInterceptor.java#L119-124)
...
- 校验
  ...
  - **重要更新**：EXTERNAL令牌类型支持完整的Redis一致性校验，包括jti会话键和兼容旧键，支持systemId和外部用户信息，改进了令牌过期解析机制
- 过期与刷新
  - 提供"即将过期"判断工具，可用于前端主动刷新或后端自动续期策略
  - **重要更新**：增强了令牌过期解析的健壮性和准确性
- 配置
...
Note: EXTERNAL令牌支持jti会话键、systemId和外部用户信息，改进过期解析
```
...
- 外部系统Token
  ...
  - **重要更新**：外部令牌现在包含systemId、externalUserId、externalUserName等丰富信息，改进了令牌过期解析
- **重要更新**：混合认证模式
  - 文件服务支持JWT和签名认证混合模式
  - 内部调用使用JWT，外部调用可使用签名验证或直接使用EXTERNAL令牌
  - 智能会话清理，避免冲突
  - 拦截器通过@Component注解自动注册，提高可维护性
- **重要更新**：用户注册流程优化，自动设置最后登录时间
...
AS-->>AC : "返回Token与用户信息"
AC-->>FE : "登录成功"
Note over FE,RegService : 用户注册流程优化
FE->>RegService : "POST /api/auth/register"
RegService->>RegService : "创建用户并自动设置最后登录时间"
RegService-->>FE : "返回注册成功"
...
- 过滤器依赖
  ...
  - **重要更新**：文件服务过滤器现在支持完整的EXTERNAL令牌类型验证，包含systemId和外部用户信息，改进了令牌过期解析
- 控制器与服务
...
RegOpt["用户注册优化"] --> LastLogin["自动设置最后登录时间"]
```
...
- 常见错误
  ...
  - **重要更新**：EXTERNAL令牌访问失败：检查文件服务过滤器是否正确配置接受EXTERNAL令牌类型，确认systemId字段是否正确传递，检查令牌过期解析逻辑
  ...
  - **重要更新**：用户注册问题：检查最后登录时间字段是否自动设置
  - **重要更新**：令牌过期解析问题：检查令牌过期时间解析逻辑的健壮性
- 定位步骤
  ...
  - **重要更新**：检查用户注册流程中最后登录时间的自动设置逻辑
  - **重要更新**：检查令牌过期解析机制的异常处理
...
## 结论
本模块以JWT为核心，结合Redis实现无状态但可控的认证体系，并通过AOP与注解简化鉴权开发。RSA公钥加密传输与密钥轮换保障敏感数据安全，角色与权限缓存提升性能与可扩展性。**重要更新**：系统现已支持多租户场景，通过systemId字段实现系统级数据隔离，大大增强了系统的扩展性和安全性。**重要更新**：外部令牌生成得到了显著增强，支持systemId、externalUserId、externalUserName等丰富字段，为外部系统集成提供了更强大的支持，同时改进了令牌过期解析机制的健壮性。**重要更新**：统一了用户标识处理，将所有MetaObjectHandler实现和操作日志切面统一使用getUsername()方法，确保了跨模块的用户标识一致性。**重要更新**：优化了用户注册流程，自动设置最后登录时间字段，提升了用户体验和系统一致性。**更新的组件化注册方式使ExternalSignatureInterceptor和FileSignatureInterceptor通过@Component注解自动注册，简化了配置管理，提高了代码的可维护性**。新增的签名验证拦截器提供了灵活的外部系统集成方案，JSON转义安全增强提升了系统的整体安全性，智能会话清理逻辑避免了上下文污染问题。**最重要的更新是增强了多租户支持和外部系统集成能力，通过systemId字段和增强的外部令牌结构，为系统的企业级应用奠定了坚实的基础，同时改进了令牌过期解析机制的可靠性和性能**。建议在部署中严格配置密钥、开启HTTPS、完善监控与审计，持续优化用户体验与安全基线。
...
- 安全配置建议
  ...
  - **重要更新**：配置令牌过期解析机制的健壮性处理
  - **重要更新**：验证用户注册流程中最后登录时间的自动设置逻辑
- 常见问题解决
  - 跨域问题：确保CORS配置允许必要来源与方法
  - 权限缺失：检查登录时是否正确缓存权限与角色
  - 登出不生效：确认登出接口是否被@RequireLogin保护且Redis键正确删除
  - **重要更新**：EXTERNAL令牌访问失败：检查文件服务过滤器配置和令牌类型设置，确认systemId字段正确传递，检查令牌过期解析逻辑
  ...
  - **重要更新**：用户注册问题：检查最后登录时间字段的自动设置逻辑
  - **重要更新**：令牌过期解析问题：检查过期时间解析的健壮性和异常处理
