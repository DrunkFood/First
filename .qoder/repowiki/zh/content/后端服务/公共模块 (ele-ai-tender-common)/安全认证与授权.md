...
## 更新摘要
**变更内容**   
- **重要更新**：文件服务JWT认证过滤器现已支持EXTERNAL令牌类型，允许外部系统使用Bearer令牌直接访问文件服务资源
- 文件服务Web配置中的JwtAuthenticationFilter现在接受EXTERNAL、INTERNAL和SERVICE三种令牌类型
- 增强了文件服务的安全认证能力，支持更灵活的外部系统集成方案
- 保持了原有的签名验证拦截器组件化注册机制
...
- **重要更新**：文件服务现支持EXTERNAL令牌类型，允许外部系统使用Bearer令牌直接访问文件资源
- **更新**：拦截器组件化注册，采用Spring Boot自动装配机制
- 安全配置示例与常见问题解决方案
...
- **重要更新**：文件服务Web配置现已支持EXTERNAL令牌类型，提供更灵活的外部系统集成方案
...
## 核心组件
- JwtAuthenticationFilter：统一鉴权入口，负责从请求中提取Token、校验签名、按类型进行Redis一致性校验，并将登录用户信息写入线程上下文。**已更新**：支持EXTERNAL令牌类型的完整验证流程。
- SecurityContextHolder：基于ThreadLocal的用户上下文容器，提供便捷方法获取当前用户ID、用户名、真实姓名、是否管理员等。
- LoginUser：承载当前登录主体信息，包括内部用户、外部系统用户、服务间调用等不同场景的字段。
- RequireLogin注解与RequireLoginAspect：声明式强制登录检查，未登录直接拒绝访问。
- JwtUtil：JWT签发、解析、校验、过期判断等工具；支持内部用户、外部系统、服务间调用三类Token。
...
## 架构总览
下图展示了从HTTP请求进入，到过滤器校验、上下文填充、业务方法执行、以及登出清理的整体流程，包括组件化的签名验证拦截器和增强的EXTERNAL令牌支持。
...
JwtFilter->>Redis : "验证EXTERNAL令牌有效性"
Redis-->>JwtFilter : "验证通过"
JwtFilter->>Ctx : "设置外部用户LoginUser"
...
- 功能要点
  ...
  - **重要更新**：完整支持EXTERNAL令牌类型的验证流程，包括jti会话键和兼容旧键
- 关键行为
  - 内部用户：根据userId在Redis中比对Token
  - 外部系统：根据appKey+userId+jti（兼容旧键）在Redis中比对Token
  - 服务间调用：仅校验签名，不查Redis
  - 角色加载：内部用户从Redis角色集合加载，便于数据隔离判断管理员
  - **新增**：EXTERNAL令牌类型支持，允许外部系统直接使用Bearer令牌访问受保护资源
...
CheckSession --> |匹配| BuildExt["构建外部用户"]
...
- 生成
  ...
  - 外部系统Token：包含appKey、userId、userName、enterprise*、type=external，并附带jti
  - 服务间调用Token：type=service，serviceName作为身份标识
- 校验
  - 统一签名校验；内部/外部需与Redis中存储的Token一致；服务间仅校验签名
  - **重要更新**：EXTERNAL令牌类型支持完整的Redis一致性校验，包括jti会话键和兼容旧键
- 过期与刷新
...
- 数据隔离
  ...
  - **重要更新**：EXTERNAL令牌用户具有完整的外部用户信息，包括企业名称、企业编码等
- 声明式鉴权
...
- [FileSignatureInterceptor.java:87-93](file://ele-ai-tender-system/ele-ai-tender-file/src/main/java/com/jy/eleaitender/file/config/FileSignatureInterceptor.java#L87-L93)
- [RequireLoginAspect.java:15-21](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/aspect/RequireLoginAspect.java#L15-L21)
...
- 过滤器依赖
  ...
  - **重要更新**：文件服务过滤器现在支持完整的EXTERNAL令牌类型验证
- 控制器与服务
...
- 安全
  ...
  - **重要更新**：EXTERNAL令牌类型提供完整的企业信息和安全上下文
...
- 常见错误
  ...
  - **重要更新**：EXTERNAL令牌访问失败：检查文件服务过滤器是否正确配置接受EXTERNAL令牌类型
...
## 结论
本模块以JWT为核心，结合Redis实现无状态但可控的认证体系，并通过AOP与注解简化鉴权开发。RSA公钥加密传输与密钥轮换保障敏感数据安全，角色与权限缓存提升性能与可扩展性。**重要更新**：文件服务现已支持EXTERNAL令牌类型，允许外部系统直接使用Bearer令牌访问文件资源，大大简化了外部集成流程。**更新的组件化注册方式使ExternalSignatureInterceptor和FileSignatureInterceptor通过@Component注解自动注册，简化了配置管理，提高了代码的可维护性**。新增的签名验证拦截器提供了灵活的外部系统集成方案，JSON转义安全增强提升了系统的整体安全性，智能会话清理逻辑避免了上下文污染问题。建议在部署中严格配置密钥、开启HTTPS、完善监控与审计，持续优化用户体验与安全基线。
...
- 常见问题解决
  ...
  - **重要更新**：EXTERNAL令牌访问失败：检查文件服务过滤器配置和令牌类型设置
