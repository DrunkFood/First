...
- 自动填充 MetaObjectHandlerImpl：在插入/更新时自动写入创建/修改时间、操作人、版本与删除标识。
...
- MetaObjectHandlerImpl 在 insert/update 时自动填充时间与操作人信息，缺失用户上下文时使用默认值，保证后台任务可稳定运行。

```mermaid
flowchart TD
Start(["插入/更新入口"]) --> CheckCtx["获取当前用户上下文"]
CheckCtx --> FillInsert{"是否为插入?"}
FillInsert --> |是| DoInsert["填充创建/修改时间、操作人、版本、删除标识"]
FillInsert --> |否| DoUpdate["仅填充修改时间、操作人"]
DoInsert --> End(["完成"])
DoUpdate --> End
```
...
- 模块内依赖：
  ...
  - MetaObjectHandlerImpl 依赖安全上下文填充审计字段。
- 外部依赖：
...
## 结论
本项目在数据访问层通过 BaseEntity 与自动填充实现了统一的审计与并发控制；借助 DataScope 注解与拦截器，完成了基于 create_id 的行级数据隔离，并兼容管理员与无上下文场景；通过 MyBatis-Plus 插件链提供了分页与乐观锁能力。整体设计清晰、可扩展性强，适合在多租户与多角色场景下演进。
