...
- [TextOptimizeParams.java](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/src/main/java/com/jy/eleaitender/common/interaction/dto\param\TextOptimizeParams.java)
...
## 更新摘要
**变更内容**   
- 新增自定义优化要求支持，用户可输入特定优化需求
- 默认提供专业增强设置作为优化要求的默认值
- 前端编辑器支持流式优化体验，包含错误恢复机制
- 后端参数模型增加requirement字段支持
...
**最新更新**：文本优化器现已支持内容输入和自定义优化要求，默认提供专业增强设置，为用户提供更灵活的优化选项。
...
图表来源
- [TextOptimizer.java:39-60](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/processor/generator/TextOptimizer.java#L39-L60)
...
- [PromptBuilder.java:23-27](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/processor/prompt/PromptBuilder.java#L23-L27)
...
- 任务参数（TextOptimizeParams）：承载待优化内容与可选优化要求，支持自定义优化需求。
- 请求DTO（OptimizeRequest）：对外暴露的文本优化请求体，包含content和requirement字段。
...
- [UserPromptTemplates.java:14-21](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/processor/prompt/UserPromptTemplates.java#L14-L21)
- [SystemPromptTemplates.java:1-497](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/processor/prompt/SystemPromptTemplates.java#L1-L497)
- [TextOptimizeParams.java:8-15](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/ai/TextOptimizeParams.java#L8-L15)
- [OptimizeRequest.java:12-23](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/dto/request/OptimizeRequest.java#L12-L23)
- [RequirementEditor.vue:232-271](file://ele-ai-tender-frontend/src/views/requirement/RequirementEditor.vue#L232-L271)
...
- 文本优化：
  ...
  - **新增功能**：支持自定义优化要求，当requirement为空时自动填充默认值。
- 扩展点：新增优化维度（如术语规范化、风格统一）可在模板中追加约束。

**更新**：PromptBuilder现在支持requirement参数的默认值处理，确保即使未提供优化要求也能获得专业的优化效果。
...
### 数据模型与接口契约
- 任务参数（TextOptimizeParams）：content、requirement（可选，默认"提升专业性和规范性"）。
- 请求体（OptimizeRequest）：content（必填）、requirement（可选）、projectId。
- 返回结果：JSON字符串{"optimizedContent": "..."}。

**更新**：TextOptimizeParams和OptimizeRequest都增加了requirement字段支持，允许用户指定特定的优化需求。

章节来源
- [TextOptimizeParams.java:8-15](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/dto/ai/TextOptimizeParams.java#L8-L15)
- [TextOptimizeParams.java:8-18](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/src/main/java/com/jy/eleaitender/common/interaction/dto\param\TextOptimizeParams.java#L8-L18)
- [OptimizeRequest.java:12-23](file://ele-ai-tender-system/ele-ai-tender-ai/src/main/java/com/jy/eleaitender/ai/dto/request/OptimizeRequest.java#L12-L23)

### 前端集成与用户体验
- 编辑器在优化过程中实时追加流式内容，失败时恢复原始文本，成功时提示完成。
- 支持选中替换：当原文已被修改则提示手动替换，存在多处相同内容时仅替换第一处。
- **新增功能**：AI优化按钮支持流式输出，提供更好的用户体验。

**更新**：前端编辑器现在支持完整的AI优化流程，包括错误处理和内容恢复机制。

章节来源
- [RequirementEditor.vue:232-271](file://ele-ai-tender-frontend/src/views/requirement/RequirementEditor.vue#L232-L271)
- [ai.ts:16-17](file://ele-ai-tender-frontend/src/api/ai.ts#L16-L17)
...
- 调优手段：
  ...
  - **新增**：利用自定义优化要求功能，针对不同场景提供专门的优化指令。
- 监控指标：
  - 成功率、平均耗时、失败原因分布、内容异常率。
  - 前后端日志与调用记录关联追踪。

**更新**：现在可以通过自定义优化要求来精确控制优化行为，满足不同业务场景的需求。
...
## 结论
文本优化器以任务化编排为核心，结合提示词工程与模型路由，实现了稳定、可扩展的AI文本优化能力。通过并发控制、超时保护与调用记录，系统在大规模使用中具备良好的可观测性与鲁棒性。配合前端编辑器的流式体验与版本对比，以及模型路由规则的灵活配置，能够满足招标文件编制中对语法修正、语义润色、风格统一与术语规范化的综合需求。

**最新改进**：新增的自定义优化要求和默认专业增强设置功能，进一步提升了系统的灵活性和易用性，使文本优化更加贴合用户的实际需求。后续可在术语库与风格指南的引入、分块优化与质量评分等方面持续演进。