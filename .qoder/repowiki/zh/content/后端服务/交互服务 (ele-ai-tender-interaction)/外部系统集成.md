...
- [AiExternalAuthClient.java](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiExternalAuthClient.java)
- [AiExternalUserInfoClient.java](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiExternalUserInfoClient.java)
...
## 更新摘要
**变更内容**   
- 外部集成客户端类已重命名为更具标识性的AI相关名称：ExternalAuthClient改为AiExternalAuthClient，ExternalUserInfoClient改为AiExternalUserInfoClient，FileClient改为AiFileClient
- 新增AiTaskClient客户端类，提供AI任务创建、查询和状态查询能力
- 更新了自动配置类中的Bean注册，使用新的AI相关客户端类名
- 这些重命名提高了代码的可读性和AI功能标识性，同时保持了向后兼容性
...
**更新** 外部集成客户端类已完成重命名，提升了代码的可读性和AI功能标识性

#### 重命名详情
- `ExternalAuthClient` → `AiExternalAuthClient`：外部认证客户端
- `ExternalUserInfoClient` → `AiExternalUserInfoClient`：外部用户信息客户端  
- `FileClient` → `AiFileClient`：文件客户端
- 新增 `AiTaskClient`：AI任务客户端

#### 新客户端类架构
```mermaid
classDiagram
class AiExternalAuthClient {
+getExternalToken(request) ExternalTokenResponse
-restTemplate : RestTemplate
-properties : EleAiTenderInteractionProperties
-signer : InteractionRequestSigner
}
class AiExternalUserInfoClient {
+getCurrentExternalUser(authorization) ExternalUserInfoResponse
-restTemplate : RestTemplate
-properties : EleAiTenderInteractionProperties
-signer : InteractionRequestSigner
}
class AiFileClient {
+getFileInfo(authorization, fileId) InteractionFileInfoResponse
+downloadFile(authorization, fileId) InteractionFileDownloadResponse
+uploadFile(authorization, content, fileName, bizType) InteractionFileUploadResponse
-restTemplate : RestTemplate
-properties : EleAiTenderInteractionProperties
}
class AiTaskClient {
+createTask(authorization, request) AiTaskCreateResponse
+getTask(authorization, taskId) AiTaskQueryResponse
+getTaskStatus(authorization, taskId) AiTaskQueryResponse
-restTemplate : RestTemplate
-properties : EleAiTenderInteractionProperties
}
AiExternalAuthClient --> InteractionRequestSigner
AiExternalUserInfoClient --> InteractionRequestSigner
```

**图表来源**
- [AiExternalAuthClient.java:26-68](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiExternalAuthClient.java#L26-L68)
- [AiExternalUserInfoClient.java:24-64](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiExternalUserInfoClient.java#L24-L64)
- [AiFileClient.java:35-203](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiFileClient.java#L35-L203)
- [AiTaskClient.java:26-113](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiTaskClient.java#L26-L113)

#### 自动配置更新
自动配置类已更新为注册新的AI相关客户端Bean：

```mermaid
sequenceDiagram
participant Config as "自动配置类"
participant BeanFactory as "Bean工厂"
participant AuthClient as "AiExternalAuthClient"
participant UserInfoClient as "AiExternalUserInfoClient"
participant FileClient as "AiFileClient"
participant TaskClient as "AiTaskClient"
Config->>BeanFactory : 注册 aiExternalAuthClient
BeanFactory->>AuthClient : 创建实例(注入RestTemplate, Signer, Properties)
Config->>BeanFactory : 注册 aiExternalUserInfoClient
BeanFactory->>UserInfoClient : 创建实例(注入RestTemplate, Signer, Properties)
Config->>BeanFactory : 注册 aiFileClient
BeanFactory->>FileClient : 创建实例(注入RestTemplate, Properties)
Config->>BeanFactory : 注册 aiTaskClient
BeanFactory->>TaskClient : 创建实例(注入RestTemplate, Properties)
```

**图表来源**
- [EleAiTenderInteractionAutoConfiguration.java:56-77](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-autoconfigure/src/main/java/com/jy/eleaitender/interaction/autoconfigure/EleAiTenderInteractionAutoConfiguration.java#L56-L77)
...
客户端类的AI相关重命名进一步提升了代码的可读性和功能标识性，使AI功能的集成更加清晰明确。新增的AiTaskClient为AI任务管理提供了完整的客户端支持，完善了整个外部集成的能力体系。
