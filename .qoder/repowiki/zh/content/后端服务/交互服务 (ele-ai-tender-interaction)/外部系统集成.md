...
- [AiPolicyFileClient.java](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/client/AiPolicyFileClient.java)
...
## 更新摘要
**变更内容**   
- 新增政策文件查询客户端(AiPolicyFileClient)，提供统一的政策文件查询能力
- 新增政策文件相关数据结构(InteractionPolicyFileVO, PolicyFileQueryResponse)，支持政策文件的标准化数据传输
- 增强外部系统集成能力，扩展了AI相关的业务接口支持
- 完善了政策文件查询的业务流程和数据流转机制
...
**最新更新**：系统已实现外部系统用户的自动创建和管理功能，大幅简化了外部系统的接入流程，提升了用户体验和系统安全性。同时新增了政策文件查询客户端和相关数据结构，增强了AI相关业务接口的支持能力。
...
- [ele-ai-tender-common-interaction/pom.xml](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-common-interaction/pom.xml)
...
- **交互契约层（common-interaction）**
  ...
  - 典型内容：投标解密提交/结果回调、标书推送、CA密钥信息查询、项目基础信息查询、身份上下文、文件上传下载响应、政策文件查询等
- **交互核心层（interaction-core）**
  - 职责：封装HTTP客户端、签名计算、参数校验、响应提取、错误映射等通用能力；提供面向业务的Client类
  - 典型内容：RestTemplate工厂、签名工具、验证工具、结果提取器、属性配置类、政策文件查询客户端
- **自动装配层（autoconfigure）**
...
## 架构总览
外部系统集成整体遵循"管理层—契约层—核心层—装配层—启动器"的分层架构，通过统一的RestTemplate工厂与拦截器完成请求签名、参数校验、响应解析与错误映射，上层业务仅关注领域语义。**新增的外部系统用户自动管理机制确保了每个外部系统都有独立的用户账户和权限控制，同时新增的政策文件查询客户端进一步扩展了AI相关业务接口的支持能力。**

```mermaid
graph TB
Manager["外部系统管理层<br/>用户自动创建/密钥管理"]
Client["业务客户端<br/>调用方服务"]
PolicyClient["政策文件查询客户端<br/>AiPolicyFileClient"]
...
**新增** 系统新增了政策文件查询客户端(AiPolicyFileClient)，提供统一的政策文件查询能力，支持AI相关的业务需求

#### 政策文件查询客户端架构
```mermaid
classDiagram
class AiPolicyFileClient {
+queryPolicyFiles(queryRequest) PolicyFileQueryResponse
+getPolicyFileDetail(fileId) InteractionPolicyFileVO
+searchPolicyFiles(keyword) InteractionPolicyFileVO[]
}
...
+InteractionPolicyFileVO[] files
+Integer total
+Integer page
+Integer pageSize
}
AiPolicyFileClient --> InteractionPolicyFileVO
AiPolicyFileClient --> PolicyFileQueryResponse
```
...
participant Client as "AiPolicyFileClient"
participant RT as "RestTemplate工厂"
participant Sig as "签名拦截器"
participant TP as "第三方平台"
AI->>Client : "查询政策文件(queryRequest)"
Client->>RT : "创建HTTP请求"
RT->>Sig : "附加签名头/时间戳/Nonce"
Sig->>TP : "GET /policy/files?keyword=..."
TP-->>Client : "返回政策文件列表"
Client-->>AI : "返回PolicyFileQueryResponse"
AI->>Client : "获取政策文件详情(fileId)"
Client->>TP : "GET /policy/file/{fileId}"
TP-->>Client : "返回政策文件详情"
Client-->>AI : "返回InteractionPolicyFileVO"
```
...
#### 数据结构设计
- **InteractionPolicyFileVO**：政策文件值对象，包含文件标识、标题、内容、发布日期、分类等基本信息
- **PolicyFileQueryResponse**：政策文件查询响应，包含文件列表、总数、分页信息等元数据
- **查询请求**：支持关键词搜索、分类过滤、时间范围筛选等条件查询
...
- **外部系统用户问题排查**
  ...
  - 检查用户状态是否正常（启用状态）
- **政策文件查询问题排查**
  - 检查AiPolicyFileClient的连接配置和网络连通性
  - 验证政策文件查询接口的参数格式和必填字段
  - 监控政策文件查询的响应时间和成功率
  - 检查政策文件数据的完整性和一致性
...
**新增功能**：政策文件查询客户端(AiPolicyFileClient)的引入进一步增强了AI相关业务接口的支持能力，提供了统一的政策文件查询接口和标准化的数据结构。这不仅简化了政策文件的管理和使用，还为AI智能分析提供了丰富的数据源支持。
...
- **政策文件查询**
  - 查询政策文件：GET /policy/files?keyword=...&category=...；返回PolicyFileQueryResponse
  - 获取政策文件详情：GET /policy/file/{fileId}；返回InteractionPolicyFileVO
  - 搜索政策文件：GET /policy/search?keyword=...&dateRange=...；返回匹配的政策文件列表
- **外部认证**
...
**图表来源**
- [OutboundLogInterceptor.java:18-75](file://ele-ai-tender-system/ele-ai-tender-interaction/ele-ai-tender-interaction-core/src/main/java/com/jy/eleaitender/interaction/core/support/OutboundLogInterceptor.java#L18-75)
...
**章节来源**
- [AccessLogProperties.java:11-22](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/logging/AccessLogProperties.java#L11-22)
- [LoggingAutoConfiguration.java:29-53](file://ele-ai-tender-system/ele-ai-tender-common/src/main/java/com/jy/eleaitender/common/logging/LoggingAutoConfiguration.java#L29-53)