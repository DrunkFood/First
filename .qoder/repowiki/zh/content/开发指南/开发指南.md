...
- [ele-ai-tender-interaction/pom.xml](file://ele-ai-tender-system/ele-ai-tender-interaction/pom.xml)
...
## 更新摘要
**变更内容**   
- 新增Maven分发管理配置章节，详细说明artifact发布机制
- 更新依赖与版本管理章节，补充集中式版本管理策略
- 增强构建与部署流程说明，包含自动化发布工作流
- 完善本地开发环境配置，包含Maven仓库设置指导
...
- **新增**：统一的dependencyManagement配置确保依赖版本一致性
...
### 集中式版本管理策略
项目采用分层依赖管理架构，通过父POM统一管理所有第三方依赖版本：

**父POM依赖管理** (`ele-ai-tender-system/pom.xml`)
- 定义所有第三方库的版本号属性（如 `${spring-boot.version}`、`${mybatis-plus.version}`）
- 在 `<dependencyManagement>` 中集中声明依赖版本
- 子模块仅需引用 `groupId` 和 `artifactId`，无需指定版本号

**特殊模块版本覆盖** (`ele-ai-tender-interaction/pom.xml`)
- 交互模块保持JDK 8兼容性，覆盖Spring Boot版本为2.7.18
- 通过局部 `dependencyManagement` 实现版本隔离

### 自动化制品发布配置
**Maven分发管理** (`ele-ai-tender-interaction/pom.xml`)
```xml
<distributionManagement>
    <!--正式版本-->
    <repository>
        <id>zbd-purchase-releases</id>
        <name>zbd-purchase-releases</name>
        <url>http://10.11.20.50:15051/repository/zbd-purchase-releases/</url>
    </repository>
    <!--快照-->
    <snapshotRepository>
        <id>zbd-purchase-snapshots</id>
        <name>zbd-purchase-snapshots</name>
        <url>http://10.11.20.50:15051/repository/zbd-purchase-snapshots/</url>
    </snapshotRepository>
</distributionManagement>
```

**发布工作流程**
- 正式版本发布至 `zbd-purchase-releases` 仓库
- 快照版本发布至 `zbd-purchase-snapshots` 仓库
- 需要配置Maven `settings.xml` 中的server认证信息

**本地开发环境配置**
需要在用户目录下创建 `.m2/settings.xml` 文件：
```xml
<settings>
    <servers>
        <server>
...
# 发布正式版本
mvn clean deploy -DskipTests -DaltDeploymentRepository=zbd-purchase-releases::default::http://10.11.20.50:15051/repository/zbd-purchase-releases/
```
...
```bash
# 构建单个模块及其依赖
mvn -pl ele-ai-tender-core -am package
...
### CI/CD流水线
项目使用Jenkins进行自动化构建和部署：

**核心服务流水线** (`Jenkinsfile-core.groovy`)
- 自动检出代码、安装依赖、执行构建
- SSH传输JAR包到目标服务器
- 健康检查验证服务启动状态

**AI服务流水线** (`Jenkinsfile-ai.groovy`)
- 类似的核心服务构建流程
- 针对AI服务特性优化内存配置（-Xmx2G）

**流水线阶段**
1. **Checkout** - 代码拉取和环境准备
2. **Build** - Maven构建和打包
3. **Deploy** - 部署到目标服务器
4. **Check** - 健康检查和验证
...
- **新增**：制品发布
  - 公共模块变更后需执行 `mvn deploy` 发布到私有仓库
  - 确保Maven settings.xml配置正确，包含仓库认证信息
...
- **新增**：Maven制品发布失败
  - 现象：deploy命令执行失败，提示认证错误或网络超时
  - 处理：检查 `.m2/settings.xml` 中的server配置，确认用户名密码正确，网络可达私有仓库地址
- **新增**：依赖版本冲突
  - 现象：运行时出现ClassNotFoundException或NoSuchMethodError
  - 处理：检查父POM中dependencyManagement配置的版本，确保子模块未显式指定冲突版本
...
## 结论
通过统一的编码规范、清晰的模块边界、集中的依赖管理与完善的异常/日志/追踪机制，本项目能够在保证质量的前提下持续演进。**新增的Maven分发管理机制进一步提升了构建效率和部署可靠性**。建议在迭代中坚持"代码优先、文档同步"的原则，配合严格的测试与审查流程，持续提升交付效率与稳定性。